...
import com.example.SpringAndMockitoApplication1.exception.EmployeeEntryError;
...
public class EmployeeServiceImpl implements EmployeeService {
    ...
    public Employee add(String firstName, String lastName, Integer departmentNumber, Integer salary) {
        ...
        if (employees.size() >= EMPLOYEES_STORAGE_SIZE) {
            throw new pro.sky.HomeWork18.lesson25.exception.EmployeeStorageIsFullException("Хранилище заполнено!");
        }
    ...
    private void checkTextData(String firstName, String lastName) {
        if (!(isAlpha(Integer.parseInt(firstName + lastName)))) {
